...
O2 = qt.tensor(*[qt.sigmaz() if i == n-1 else qt.identity(2) for i in range(n)])
...
plt.rc('font', size=8)  
fig, ax = plt.subplots()
V = np.fft.fft(to_pauli(O).full().T[0])
V = V/np.linalg.norm(V)

V2 = np.fft.fft(to_pauli(O2).full().T[0])
...
p6 = plt.fill_between(np.arange(len(pauli_op_names)), [(v*v.conj()).real for v in V2], color='yellow')
